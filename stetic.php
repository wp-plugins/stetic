...
if(!class_exists('Stetic'))
{
	class Stetic
	{
		...
		public static function stats_page()
		{
			...
			{
				$options['stetic_token'] = $_POST['stetic_token'];
				$options['stetic_api_key'] = $_POST['stetic_api_key'];
				$options['stetic_show_counter'] = $_POST['stetic_show_counter'];
				$options['stetic_disable_tracking'] = $_POST['stetic_disable_tracking'];
				update_option('stetic', $options);
			}
			
			$options  = get_option('stetic');
			
			if($_GET['tab'] == 'settings' || self::$tab == 'settings' || !$options['stetic_api_key'])
			{
				?>
				<div class="wrap">
					<h2>Stetic Configuration</h2>
					<?php self::the_tabs(); ?>
					<form action="" method="post" id="stetic-conf">
						<table class="form-table">
							<tr>
								<td colspan="2">
									<label for="stetic_project_id">Stetic Project Token:</label><br/>
									<input size="50" type="text" id="stetic_token" name="stetic_token" <?php echo 'value="'.$options['stetic_token'].'" '; ?>/><br/>
							...
							<tr>
								<td colspan="2">
									...
									<input size="50" type="text" id="stetic_api_key" name="stetic_api_key" <?php echo 'value="'.$options['stetic_api_key'].'" '; ?>/><br/>
							...
							<tr>
								<td colspan="2">
									...
									<input size="50" type="checkbox" id="stetic_show_counter" name="stetic_show_counter" value="1" <?php echo ($options['stetic_show_counter'] == "1") ? 'checked="checked"' : ""; ?>/> Show Counter<br/>
							...
							<tr>
								<td colspan="2">
									...
									<input type="checkbox" id="stetic_disable_tracking" name="stetic_disable_tracking" value="1" <?php echo ($options['stetic_disable_tracking'] == "1") ? 'checked="checked"' : ""; ?>/> Disable Tracking<br/>
			...
			else
			{
				...
				jQuery(document).ready(function() { 
					fs = new fourStats('<?php echo $options['stetic_project_id']; ?>', '<?php echo $options['stetic_token']; ?>', '<?php echo $options['stetic_api_key']; ?>', '', '<?php echo date("D, d M Y H:i:s"); ?>');
					fs.statsPage();
				});				
				</script>
				<?
		...
		*/
		...
		public static function tracking_code_header()
		{
			...
			{
				$id_string = ( isset($options['stetic_project_id']) && $options['stetic_project_id'] ) ? "siteId='{$options['stetic_project_id']}'" : "token='{$options['stetic_token']}'";
